...
app.service('getCandidatoService',function ($http) {
  var urlBase='http://localhost:8080/votweet-backend-master'
  this.getAll=function (success,failure) {
        $http.get('http://localhost:8080/votweet-backend-master/candidatos')
        .success(success)
        .failure(failure);
    };
...
app.controller('controlCdtos',function ($scope,$http) {
  ...
  $http({
      method: 'GET',
      url: 'http://localhost:8080/votweet-backend-master/candidatos'
   }).then(function (data){
